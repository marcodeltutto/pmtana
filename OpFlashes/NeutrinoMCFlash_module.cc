...
{
  mf::LogDebug("NeutrinoMCFlash") << "***** NeutrinoMCFlash starts." << std::endl;
  ...
  for (size_t n = 0; n < evt_mctruth_h->size(); n++) {
    ...
    mf::LogDebug("NeutrinoMCFlash") << "Origin: " << evt_mctruth.Origin() << std::endl;
    ...
    for (int p = 0; p < evt_mctruth.NParticles(); p++) {
      ...
      mf::LogDebug("NeutrinoMCFlash") << "Particle time: " << par.Trajectory().T(0) << std::endl;
      mf::LogDebug("NeutrinoMCFlash") << "    converted: " << ts->G4ToElecTime(par.Trajectory().T(0)) - trig_time << std::endl;
      mf::LogDebug("NeutrinoMCFlash") << "new Particle time: " << par.T() << std::endl;
      mf::LogDebug("NeutrinoMCFlash") << "new    converted: " << ts->G4ToElecTime(par.T()) - trig_time << std::endl;
      nuTime = par.T();//ts->G4ToElecTime(par.T()) - trig_time;
      /*
      simb::MCTrajectory::const_iterator i = par.Trajectory().begin();
      for (int k = 1; true; k++){

        std::cout << "z is " << par.Trajectory().Z(k) << std::endl;
        std::advance(i,k);
        if (i == par.Trajectory().end()){
          std::cout << "At the end, k = " << k << std::endl;
        //  break;
        }
        if (k==100) break;
      }
  ...
  mf::LogDebug("NeutrinoMCFlash") << "Neutrino estimated interaction time: "  << nuTime << std::endl; 
  ...
  for(size_t opdet=0; opdet<32; ++opdet) {
    ...
    mf::LogDebug("NeutrinoMCFlash") << "Opdet " << opdet << std::endl;

    for(auto const& oneph : simph) {

      if (oneph.Time > nuTime + 100) continue;
      if( oneph.Time > nuTime - 1000){ 
        mf::LogDebug("NeutrinoMCFlash") << " photon time " << oneph.Time << std::endl;
  ...
  for(size_t opdet=0; opdet<32; ++opdet) {
    ...
    std::cout << "OOOOOOO pmt_v[0][opdet]" << pmt_v[0][opdet] << std::endl;
  }
  ...
  mf::LogDebug("NeutrinoMCFlash") << "***** NeutrinoMCFlash ends." << std::endl;
